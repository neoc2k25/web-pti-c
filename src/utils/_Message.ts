export const MsgPositron = (step: number) => {
  ...
  if (step === 1) {
    ...
    message =
      "Saat positron berlangsung, kami juga dipimpin oleh seorang dewan komunal lohh... supaya komunikasi antara maba dan mentor berjalan dengan lancar...🌟👑";
  } else if (step === 6) {
    message = "Kenalin, MUHAMMAD ZAKY ZHALIFUNNAS, dia adalah dewan komunal kami dari neo-c...✨⚜️";
  } else if (step === 7) {
    message =
      "Saat positron berlangsung, kami juga harus menggunakan dresscode harian loh..👕👔";
  } else if (step === 8) {
    message =
      "Kalau ngga taat aturan, kami dimarahi kakak mentor dan dewan komunal..😡🙄";
  } else if (step === 9) {
    message =
    ...
    message =
      "Setelah ospek departemen selesai, akan ada seven segmen, orang yang bertanggungjawab mengompakkan mahasiswa di Departemen Elektro dan Informatika 🌟👑";
  } else if (step === 22) {
    message = "kenalin, THERESA NATHALIE DAVNY WOWILING, dia adalah seven segmen kami dari neo-c...✨⚜️";
  } else if (step === 23) {
    message =
      "Terima kasih telah meluangkan waktu untuk melihat animasi ini dari awal sampai selesai. Semoga bisa menghibur dan menginspirasi! 🥳🙌";
  } else {
    message = "";
  }
